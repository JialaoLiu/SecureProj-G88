...
// WebSocket URL - can be switched between WS and WSS
...
function connectWebSocket() {
  ...
  console.log('[WebSocket] Connecting to:', wsUrl.value)
  ...
  ws.connect((msg) => {
    ...
    // 监听USER_HELLO响应，表示连接已建立
    if ((msg.type === 'ACK' || msg.type === 'USER_HELLO') && !connected.value) {
      connected.value = true
      console.log('[WS] Connection established, requesting online users')
      setTimeout(() => {
        requestOnlineUsers()
        // 连接稳定后开始定期更新在线用户列表
...
.protocol-indicator.wss-secure {
  background-color: #3ba55d;
  color: white;
  font-size: 11px;
}

.protocol-indicator.ws-insecure {
